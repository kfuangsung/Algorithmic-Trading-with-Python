...
def main():
    ...
    for ticker in tqdm(my_assets, desc='Tickers', leave=True):    
        ...
        split_date = data_sql.index[-1] - dt.timedelta(days=6)
        
        tqdm.write("***** PARAMETERS OPTIMZATION *****")
        tqdm.write(f"Ticker: {ticker}")
        tqdm.write(f"Frequency: {freq}")
        tqdm.write(f"Lastest date: {data_sql.index[-1]}")
        tqdm.write(f"Split date: {split_date}")
        tqdm.write('-'*50)
        ...
        mama_results = p_umap(get_mama_return, 
                            ...
                            repeat(split_date), 
                            leave=False)
        best_mama_period = pd.DataFrame(mama_results).sort_values(by=2, ascending=False).iloc[0, 0]
        ...
        slope_results = p_umap(get_slope_return, 
                            ...
                            repeat(split_date),
                            leave=False)
        best_slope_period = pd.DataFrame(slope_results).sort_values(by=2, ascending=False).iloc[0, 0]
        ...
        tsf_results = p_umap(get_tsf_return, 
                            ...
                            repeat(split_date),
                            leave=False)
        best_tsf_period = pd.DataFrame(tsf_results).sort_values(by=2, ascending=False).iloc[0, 0]
