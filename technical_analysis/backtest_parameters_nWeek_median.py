...
def get_optim_param(data, signal, param, freq, start_date, end_date): 
    ...
    optim_param = pd.DataFrame(res).sort_values(by=1, ascending=False).iloc[:n_top,0].median()
...
def get_mama_optim(input_data, freq, param_range):
    ...
    for n in mama_periods:
        data[f"mama_{n:.1f}"], data[f"fama_{n:.1f}"] = talib.MAMA(data.close, fastlimit=n, slowlimit=n/10)
...
def get_slope_optim(input_data, freq, param_range):
    ...
    for n in slope_periods:
        data[f'signals_{n:.0f}'] = talib.LINEARREG_SLOPE(data.close, timeperiod=n).apply(np.sign)
    data.dropna(axis=0, inplace=True)
...
def get_tsf_optim(input_data, freq, param_range):
    ...
    for n in tsf_periods:
        data[f'tsf_{n:.0f}'] = talib.TSF(data.close, n)
        data[f'signals_{n:.0f}'] = np.where(data.close > data[f'tsf_{n:.0f}'], 1, -1)
    data.dropna(axis=0, inplace=True)
...
def get_aggregate_return(data, freq, ds, params):
    ...
    data_["mama"], data_["fama"] = talib.MAMA(data_.close, fastlimit=params[0], slowlimit=params[0]/10)
    data_['signals_mama'] = np.where(data_[f"mama"] > data_["fama"], 1, -1) 

    data_['signals_slope'] = talib.LINEARREG_SLOPE(data_.close, timeperiod=params[1]).apply(np.sign)

    data_['tsf'] = talib.TSF(data_.close, params[2])
    data_['signals_tsf'] = np.where(data_.close > data_['tsf'], 1, -1)
...
def main():
    args = sys.argv
    if len(args) < 3: raise ValueError("Usage python backtest_parameters_nWeek.py [number of week] [number of top params]")
    if not isinstance(int(args[1]), (int, float)): raise ValueError("[number of week] must be a number") 
    if not isinstance(int(args[1]), (int, float)): raise ValueError("[number of top params] must be a number") 
    ...
    print(f"Number of params: {n_top}")
